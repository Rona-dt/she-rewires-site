...
| KI-007 | P2 | Preview deployment | In progress | The redesign branch is committed and pushed, but the latest Vercel Preview was canceled before build because the project requires verified commits. | Create and push a signed checkpoint, then verify the Git-triggered Preview metadata. |
...
| KI-010 | P1 | Deployment workflow | In progress | The repository now documents the Git-triggered Vercel workflow, but the branch still needs a verified signed commit for the linked project setting. | Use the workflow in `AGENTS.md`; verify branch, commit signature, and deployment metadata on every Preview. |
...
- Vercel Preview is currently blocked by the linked project's verified-commit requirement; this checkpoint is intended to validate the corrected workflow.
