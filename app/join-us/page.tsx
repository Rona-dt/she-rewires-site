...
export default function JoinUsPage() {
  ...
  return (
    <div className="bg-md-background">
      <HeroSection hero={joinUsPage.hero} />

      <section className="py-14">
        <div className="container">
          <div className="mb-8 max-w-4xl">
            <p className="text-xs font-medium uppercase tracking-[0.16em] text-md-primary">Entry Paths</p>
            <h2 className="mt-2 text-3xl font-semibold text-md-onSurface md:text-4xl">Choose how you want to enter the system</h2>
          </div>
      ...
      <section className="py-14">
        <div className="container grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
          <Card>
            <CardHeader>
              <CardTitle>Tell us how you want to build with us</CardTitle>
            </CardHeader>
            <CardContent>
              <form className="space-y-6" onSubmit={handleSubmit}>
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    <label className="mb-2 block text-sm font-medium text-md-onSurfaceVariant">Name *</label>
                    <Input required value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} placeholder="Your name" />
                  </div>
                  <div>
                    <label className="mb-2 block text-sm font-medium text-md-onSurfaceVariant">City *</label>
                    <Input required value={form.city} onChange={(event) => setForm((current) => ({ ...current, city: event.target.value }))} placeholder="Your city" />
                  </div>
                </div>
                <div>
                ...
                <div>
                  <label className="mb-2 block text-sm font-medium text-md-onSurfaceVariant">Entry paths *</label>
                ...
                <div>
                  <label className="mb-2 block text-sm font-medium text-md-onSurfaceVariant">Message</label>
                  <Textarea rows={5} value={form.message} onChange={(event) => setForm((current) => ({ ...current, message: event.target.value }))} placeholder="Tell us what you want to build, contribute, or explore." />
                </div>
                <p className="text-xs text-md-onSurfaceVariant">
                  This form opens a pre-filled email draft. It does not store your information on a server yet.
                </p>
                <Button type="submit" className="w-full">
                  <Send className="h-4 w-4" /> Open email draft
                </Button>
              </form>
            </CardContent>
          </Card>
          ...
          <div className="space-y-5">
            <Card>
              <CardContent className="space-y-4 p-6">
                <h3 className="text-xl font-medium text-md-onSurface">Direct channels</h3>
            ...
            <Card>
              <CardContent className="p-6">
                <h3 className="text-xl font-medium text-md-onSurface">For partnerships</h3>
                <p className="mt-3 text-sm text-md-onSurfaceVariant">Organizations can contact the partnership team for programs, research, sponsorship, and ecosystem collaboration.</p>
