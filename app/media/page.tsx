...
import { linkedInPosts } from "@/lib/social-posts"
...
export default function MediaPage() {
  ...
  const mediaPosts = linkedInPosts.filter((post) => post.type === "media")
  ...
  return (
    <div className="bg-md-background pt-20">
      ...
      <section className="py-10">
        <div className="container">
          <h2 className="mb-8 text-3xl font-medium text-md-onSurface">{language === "en" ? "LinkedIn media feed (latest year)" : "LinkedIn 媒体内容（近一年）"}</h2>
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {mediaPosts.map((post) => (
              <Card key={post.id} className="group">
                <CardContent className="p-6">
                  <div className="mb-3 flex items-center justify-between">
                    <span className="rounded-full bg-accent/50 px-3 py-1 text-sm font-medium text-md-onSurface">LinkedIn</span>
                  ...
                  >
                    {language === "en" ? "Read on LinkedIn" : "在 LinkedIn 查看"}
